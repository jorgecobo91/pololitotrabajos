...
/**
 * Recuperación pública de publicaciones por email.
 * Cliente no tiene cuenta — todo va por link/correo.
 */
export function ClienteRecoverScreen() {
  ...
  const handleRecover = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`${APP_URL}/api/email`, {
        method: 'POST',
      ...
      <ScrollView contentContainerStyle={{ flexGrow: 1, justifyContent: 'center', paddingHorizontal: 18 }} className="bg-pt-bg">
        <View style={{ alignItems: 'center', marginBottom: 24 }}>
          ...
          <Text className="font-jakarta-regular" style={{ fontSize: 13, color: '#6B665D', textAlign: 'center', lineHeight: 20, paddingHorizontal: 16 }}>
            Si <Text className="font-jakarta-bold" style={{ color: '#1C170D' }}>{email}</Text> tiene
            publicaciones activas, te enviaremos los enlaces de acceso.
          </Text>
        </View>

        <Pressable onPress={() => navigation.goBack()} style={{ borderRadius: 14, overflow: 'hidden' }}>
          <LinearGradient
  ...
  return (
    <ScrollView contentContainerStyle={{ flexGrow: 1, justifyContent: 'center', paddingHorizontal: 18, paddingVertical: 32 }} className="bg-pt-bg">
      ...
      <Text className="font-jakarta-regular" style={{ fontSize: 13, color: '#6B665D', marginBottom: 24, lineHeight: 18 }}>
        Ingresa el correo que usaste al publicar y te enviaremos los enlaces de tus publicaciones activas.
      </Text>
      ...
      >
        ...
        >
          <Text className="font-jakarta-bold" style={{ fontSize: 14, color: '#fff' }}>
            {loading ? 'Enviando...' : 'Enviar enlaces a mi correo'}
          </Text>
        </LinearGradient>
      </Pressable>
    </ScrollView>
